class Campo:
    ...
    def incluye(self, y, x):
        ...
        return self.y <= y <= self.y + self.dim - 1 and self.x <= x <= self.x + self.dim - 1
...
def seccionar(matriz, cuadrante, silos, codigo, posSilo):
    ...
    for numeroDeCuadrante in range(4):
        if numeroDeCuadrante != numeroDeCuadranteConSilo:
            ...
            i = 0
            for c in cuadrante.obtenerCuadrantes()[numeroDeCuadrante].obtenerCuadrantes():
                if c.incluye(y, x):
...
def main(n, y, x):
    ...
    for i in range(n):
        print("[", end="")
        for j in range(n):
            print(f"{'0' if matriz[i][j] < 10 else ''}{matriz[i][j]}{',' if j < n-1 else ''}", end=" ")
        print("]")
